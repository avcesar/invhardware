...
if ARGV[0] == '-h' && ARGV[1].to_i > 0 && ARGV[1].to_i <= 254
	hi = he = ARGV[1].to_i
else
	hi = 1
	he = 254
					...
					if @host.mapdrive(@host.ip,@host.user,@host.passwd)
						if @host.leer_ID(DRIVE + FILE)
							...
							if @host.compare_idfile_with_host(@host)
								...
								@host.log("#{@host.ip}, #{@host.name}, Inconsistencia entre chasisis.inf e informacion del host, #{Time.now}")	
							end
						else
